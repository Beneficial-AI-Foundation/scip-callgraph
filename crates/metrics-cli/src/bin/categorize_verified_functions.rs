...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() != 3 {
        eprintln!("Usage: {} <input_atoms_json> <output_categorized_json>", args[0]);
        eprintln!();
    ...
    let categorized: Vec<VerificationCategory> = atoms
        .iter()
        .map(categorize_function)
        .collect();
    
    // Compute statistics
    let total = categorized.len();
    let with_proof = categorized.iter().filter(|c| c.has_proof_block).count();
    let trivially_verified = categorized.iter().filter(|c| c.is_trivially_verified).count();
    let with_assume_false = categorized.iter().filter(|c| c.has_assume_false).count();
    let with_specs = categorized.iter().filter(|c| c.has_requires || c.has_ensures).count();
    ...
    println!("  1. Verified with proof blocks:   {} ({:.1}%)", 
             with_proof, (with_proof as f64 / total as f64) * 100.0);
    println!("  2. Trivially verified (no proof): {} ({:.1}%)", 
             trivially_verified, (trivially_verified as f64 / total as f64) * 100.0);
    println!("  3. With assume(false):           {} ({:.1}%)", 
             with_assume_false, (with_assume_false as f64 / total as f64) * 100.0);
    println!();
    println!("  TOTAL VERIFIED: {} ({:.1}%)", 
             verified_total, (verified_total as f64 / total as f64) * 100.0);
    println!();
    println!("Other categories:");
    println!("  Functions with specs:            {} ({:.1}%)", 
             with_specs, (with_specs as f64 / total as f64) * 100.0);
    println!("  Unspecified:                     {} ({:.1}%)", 
             total - with_specs, ((total - with_specs) as f64 / total as f64) * 100.0);
    println!();
    println!("═══════════════════════════════════════════════════════════════");
    println!();
    ...
    for (cat, count) in categories {
        println!("  {}: {} ({:.1}%)", 
                 cat, count, (*count as f64 / total as f64) * 100.0);
    }
    ...
    let trivial_examples: Vec<_> = categorized.iter()
    ...
    for ex in trivial_examples {
        ...
        println!("    requires: {}, ensures: {}", ex.requires_count, ex.ensures_count);
    }
    
    println!();
    println!("Functions with proof blocks (first 10):");
    let proof_examples: Vec<_> = categorized.iter()
