...
fn load_rca_metrics(
    ...
    println!(
        "Loaded metrics for {} functions from RCA JSONs",
        all_functions.len()
    );
    Ok(all_functions)
}
...
fn find_function_metrics(
    ...
    if debug {
        ...
        let similar: Vec<&String> = rca_metrics
            .keys()
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() < 4 {
        eprintln!(
            "Usage: {} <input_csv> <rca_json_dir> <output_csv> [--debug]",
            args[0]
        );
        eprintln!("\nExample:");
        eprintln!(
            "  {} functions_to_track.csv curve25519-dalek/ output.csv",
            args[0]
        );
        eprintln!(
            "  {} functions_to_track.csv curve25519-dalek/ output.csv --debug",
            args[0]
        );
        std::process::exit(1);
    }
    ...
    for csv_func in &functions {
        let metrics =
            find_function_metrics(&csv_func.function, &csv_func.module, &rca_metrics, debug);
    ...
    println!(
        ...
        matched,
        functions.len(),
        (matched as f64 / functions.len() as f64) * 100.0
    );
