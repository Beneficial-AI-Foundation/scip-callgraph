...
fn load_atoms_functions(atoms_path: &str) -> HashMap<String, HashSet<String>> {
    ...
    let content = fs::read_to_string(atoms_path).expect("Failed to read atoms JSON");

    let atoms: Vec<Atom> = serde_json::from_str(&content).expect("Failed to parse atoms JSON");
    ...
    if let Ok(content) = fs::read_to_string(&full_path) {
        ...
        if let (Some(start), Some(end)) = (start_line, end_line) {
            ...
            if start > 0 && end <= lines.len() {
                ...
                if function_text.contains(&pattern1)
                    || function_text.contains(&pattern2)
                    || function_text.contains(&pattern3)
                {
                    return true;
                }
            }
...
fn main() {
    ...
    if args.len() != 4 {
        eprintln!(
            "Usage: {} <rca_json_dir> <vanilla_source_dir> <atoms_json>",
            args[0]
        );
        eprintln!("\nArguments:");
        eprintln!(
            "  <rca_json_dir>        - Directory with rust-code-analysis JSONs (from vanilla)"
        );
        ...
        eprintln!(
            "    /home/lacra/git_repos/curve25519-dalek/curve25519-dalek \\  # Vanilla source"
        );
