...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() != 4 {
        eprintln!(
            "Usage: {} <verification_categories_json> <tracked_csv> <output_json>",
            args[0]
        );
        eprintln!();
    ...
    println!(
        "Loading verification categories from {}...",
        categories_path
    );
    ...
    for cat in &categories {
        ...
        let is_verified = (cat.category == "trivially_verified"
            || cat.category == "verified_with_proof")
        ...
        let is_tracked = tracked.contains(&cat.display_name) || tracked.contains(&module_function);
    ...
    let trivial_untracked = untracked_verified
        .iter()
        .filter(|f| f.is_trivially_verified)
        .count();
    let proof_untracked = untracked_verified
        .iter()
        .filter(|f| f.has_proof_block)
        .count();

    // Group by file
    let mut by_file: std::collections::HashMap<String, Vec<&UntrackedVerified>> =
        std::collections::HashMap::new();
    for func in &untracked_verified {
        by_file
            .entry(func.file_name.clone())
            .or_default()
            .push(func);
    }
    ...
    println!(
        "  - Trivially verified: {} ({:.1}%)",
        trivial_untracked,
        (trivial_untracked as f64 / total_untracked as f64) * 100.0
    );
    println!(
        "  - With proof blocks: {} ({:.1}%)",
        proof_untracked,
        (proof_untracked as f64 / total_untracked as f64) * 100.0
    );
    println!();

    println!("Breakdown by file (top 15):");
    let mut file_counts: Vec<_> = by_file
        .iter()
        ...
        .take(15)
    {
        ...
        println!(
            "    requires: {}, ensures: {}",
            func.requires_count, func.ensures_count
        );
    }
