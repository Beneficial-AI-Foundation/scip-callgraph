...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "  │  ├─ Trivially verified (trivial_proof=yes): {}",
        stats.trivial_proof
    );
    println!(
        "  │  └─ With proof blocks (trivial_proof=no): {}",
        stats.with_proof_block
    );
    ...
    if stats.matched > 0 {
        ...
        println!(
            "  • {:.1}% have actual proof blocks (manual proofs)",
            proof_pct
        );
    }
