...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() != 4 {
        eprintln!(
            "Usage: {} <verification_categories_json> <input_csv> <output_csv>",
            args[0]
        );
        eprintln!();
    ...
    println!(
        "  Built lookup maps: {} by name, {} by module::function",
        by_display_name.len(),
        by_module_function.len()
    );
    ...
    for result in reader.deserialize() {
        ...
        let enriched = if let Some(cat) = category {
            ...
            EnrichedCsvRow {
                ...
                is_trivially_verified: if cat.is_trivially_verified {
                    "yes"
                } else {
                    "no"
                }
    ...
    for row in &enriched_rows {
        if row.verification_category != "not_found" {
            *category_counts
                .entry(row.verification_category.clone())
                .or_insert(0) += 1;
        }
    }
    ...
    for (cat, count) in categories {
        println!(
            ...
            cat,
            count,
            (*count as f64 / matched as f64) * 100.0
        );
    }
    ...
    println!();
    println!(
        "TOTAL VERIFIED in CSV: {} ({:.1}%)",
        total_verified,
        (total_verified as f64 / matched as f64) * 100.0
    );
    println!(
        "  - Trivially verified: {} ({:.1}%)",
        trivial,
        (*trivial as f64 / total_verified as f64) * 100.0
    );
    println!(
        "  - With proof blocks: {} ({:.1}%)",
        with_proof,
        (*with_proof as f64 / total_verified as f64) * 100.0
    );
