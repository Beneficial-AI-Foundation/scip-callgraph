...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() != 3 {
        ...
        eprintln!();
        eprintln!("Computes Halstead metrics for proof blocks including transitive lemma dependencies.");
        eprintln!();
        eprintln!("Example:");
        eprintln!("  {} \\", args[0]);
    ...
    println!("  With transitive proof metrics: {}", 
             atoms_with_proof.iter().filter(|a| a.proof_metrics.is_some()).count());
