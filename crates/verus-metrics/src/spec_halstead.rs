...
pub fn is_prose(spec: &str) -> bool {
    ...
    let prose_indicators = [
        "However,", "Thus,", "Therefore,", "i.e.", "e.g.",
        "must be clear", "should be", "cannot add", "are swapped",
        "remain unchanged", "is equivalent to", "We have that",
        "only changing", "in either case", "returns if",
        "given input", "an inverse", "each coset",
        "is the multiplicative inverse", "is zero",
        "similarly for the",
    ];
    ...
    let prose_starters = [
        "However", "Thus", "Therefore", "Given", "When", "If",
        "The ", "A ", "An ", "This ", "That ", "These ", "Those ",
        "We ", "It ", "As ", "For ", "In ", "On ", "At ",
    ];
    ...
    for starter in &prose_starters {
        if spec_trimmed.starts_with(starter) {
            ...
            if !spec_trimmed.contains("==") && !spec_trimmed.contains("!=") 
               && !spec_trimmed.contains("<=") && !spec_trimmed.contains(">=") {
                return true;
            }
        }
    }
    ...
    let operator_chars = ['=', '<', '>', '!', '&', '|', '+', '-', '*', '/', '%'];
    let operator_count = spec_trimmed.chars().filter(|c| operator_chars.contains(c)).count();
...
pub fn analyze_spec(spec_text: &str) -> Result<SpecHalsteadMetrics, String> {
    ...
    if is_prose(spec_text) {
        return Err(format!("Skipped prose/documentation: '{}'", 
            if spec_text.len() > 60 {
                format!("{}...", &spec_text[..60])
            } else {
                spec_text.to_string()
    ...
    if preprocessed.is_empty() {
        return Err(format!("Skipped non-expression clause: '{}'", 
            if spec_text.len() > 60 {
                format!("{}...", &spec_text[..60])
            } else {
                spec_text.to_string()
