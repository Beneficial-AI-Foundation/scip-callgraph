...
use rust_analyzer_test::scip_to_call_graph_json::{parse_scip_json, build_call_graph, generate_files_subgraph_dot};
use std::env;

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let args: Vec<String> = env::args().collect();
    if args.len() < 4 {
        eprintln!("Usage: {} <input-scip-json> <output-dot-file> <file-path1> [<file-path2> ...]", args[0]);
        eprintln!("Example: {} scip_data.json output.dot src/file1.rs src/file2.rs", args[0]);
        std::process::exit(1);
    }
    ...
    println!("Generating subgraph DOT file for {} files at {}...", file_paths.len(), output_path);
    ...
    match generate_files_subgraph_dot(&call_graph, &file_paths, output_path) {
        Ok(_) => {
            ...
            println!("To generate SVG, run: dot -Tsvg {} -o files_subgraph.svg", output_path);
        },
        Err(e) => {
